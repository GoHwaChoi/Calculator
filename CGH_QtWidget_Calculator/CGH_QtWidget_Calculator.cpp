...
{   
    ...
    if (strNum == "+" || strNum == "-" || strNum == "*" || strNum == "/")       //연산자 입력 시 상단 위젯에만 출력
    {
        auto strExpr = ui.lineEdit_expr->text();        //기존 수식 가져오기

        strExpr += strPrev + strNum;                    //입력한 피연산자와 연산자를 계산식에 추가

        ui.lineEdit_expr->setText(strExpr);             //계산식 누적하여 lineEdit위젯에 출력
        ui.lineEdit_num->setText("");                   //숫자 입력창에 다음 숫자가 입력되도록 문자열 초기화
        strPrev.clear();                                //입력창의 문자열 초기화
    }
    else
    {                                                   
        ...
        if (checkEndExpr == "=")                        //계산 완료. 결과값 연산 출력된 이후
        {
			...
			strPrev += strNum;                          //새로 입력된 숫자 저장
                    
			ui.lineEdit_num->setText(strPrev);          //새로 입력된 숫자 출력
            checkEndExpr.clear();                       //신규 계산 초기화 동작 완료 후 변수 초기화
            strExpr.clear();
        }
        else
        {
			strPrev += strNum;                          //계산식 입력값 누적하여 저장

			ui.lineEdit_num->setText(strPrev);          //누적된 계산식 출력
        }
